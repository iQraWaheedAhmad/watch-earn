...
async function submitDeposit(request: AuthRequest) {
  try {
    ...
    // Create pending deposit and pending referral reward (if applicable)
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Create a pending referral reward immediately so it shows up; will be paid after confirmation
